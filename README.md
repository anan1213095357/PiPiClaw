...
**让 AI 成为你的全能智能管家，原生对接 Skill-Hub 一万+ 生态技能，支持多个皮皮虾代理互联协作**
...
- **多代理互联**：多个皮皮虾实例可以相互通信、协同工作，组建成一支真正的 AI 公司——每个代理分工明确，像一个团队一样协作完成复杂任务。
...
| 🤝 **多代理协作** | 支持多个皮皮虾实例互联互通，组建 AI 公司，协同完成复杂任务 |
...
- ✅ **多代理协作** - 多个皮皮虾实例互联，组建 AI 公司协同作战

---

## 🤝 多代理协作 (AI 公司模式)

PiPiClaw 现已支持多个实例之间的互联互通，让你的 AI 助手们真正组建成一家 **AI 公司**。

**工作原理：**
- 每个皮皮虾实例在各自的机器（或端口）上独立运行，拥有自己的 Web 控制台与技能集。
- 代理之间可以通过 HTTP 接口相互调用，将子任务委派给其他实例，实现跨机器/跨进程的协同工作。
- 你可以按照职能分工启动多个实例，例如：一个负责代码开发、一个负责运维、一个负责数据分析，形成完整的 AI 工作流水线。

**快速体验多代理：**

```bash
# 终端 1：启动第一个皮皮虾（默认端口 5050）
...
然后让其中一个代理通过 HTTP 工具调用另一个代理的 Web 接口，即可实现代理间的对话与任务委派。

> 💡 **提示**：多个实例可以运行在同一台机器的不同端口，也可以分布在局域网内的不同设备上，彼此通过 IP + 端口互联。
